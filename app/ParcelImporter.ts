...
export class ParcelImporter {
  ...
  private async fetchParcelById(parcelId: string) {
    //0428000101101
    ...
    return (await resp.json()) as ParcelFeatureType;
  }

  private async fetchParcelByLocation(location: Position) {
    ...
    const resp = await fetch(urlTemplate);
    return (await resp.json()) as ParcelResponseByLocation;
  }
